...
namespace Nikki.Support.Prostreet.Framework
{
	internal static class Saver
	{
		...
		private static void WriteMaterials(BinaryWriter bw, Database.Prostreet db)
		{
			foreach (var Class in db.Materials.Collections)
				Class.Assemble(bw);
		}

		private static void WriteTPKBlocks(BinaryWriter bw, Options options, Database.Prostreet db)
		{
		...
		private static void WriteCarTypeInfos(BinaryWriter bw, Options options, Database.Prostreet db)
		{
			WritePadding(bw, options.Watermark);
		...
		private static void WriteCollisions(BinaryWriter bw, Options options, Database.Prostreet db)
		{
			long pos = 0;
			long len = 0;
		...
		private static void WriteSunInfos(BinaryWriter bw, Options options, Database.Prostreet db)
		{
			WritePadding(bw, options.Watermark);
		...
		private static void WriteTracks(BinaryWriter bw, Options options, Database.Prostreet db)
		{
			WritePadding(bw, options.Watermark);
		...
		private static void WriteCarParts(BinaryWriter bw, Options options, Database.Prostreet db) =>
			CarPartManager.Assemble(bw, options.Watermark, db);

		private static void WriteFNGroups(BinaryWriter bw, Options options, Database.Prostreet db)
		{
		...
		private static void WriteSTRBlocks(BinaryWriter bw, Options options, Database.Prostreet db)
		{

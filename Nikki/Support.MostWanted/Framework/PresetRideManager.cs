...
namespace Nikki.Support.MostWanted.Framework
{
	...
	public class PresetRideManager : Manager<PresetRide>
	{
		...
		/// <summary>
		/// Indicates required alighment when this <see cref="PresetRideManager"/> is being serialized.
		/// </summary>
		public override Alignment Alignment { get; }

		/// <summary>
		/// Gets a collection and unit element type in this <see cref="PresetRideManager"/>.
		/// </summary>
		public override Type CollectionType => typeof(PresetRide);
		...
		internal override void Assemble(BinaryWriter bw, string mark)
		{
			...
			bw.WriteEnum(BinBlockID.PresetRides);
		...
		internal override void Disassemble(BinaryReader br, Block block)
		{
			...
			if (block.BlockID != BinBlockID.PresetRides) return;

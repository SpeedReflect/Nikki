...
namespace Nikki.Support.Underground2.Framework
{
	...
	{
		/// <summary>
		/// This method generates a string dictionary for CarPart database. It is a 
		/// <see cref="String.GetHashCode()"/> to string block offset dictionary, 
		/// since HashCode in such large dictionaries is generally faster than using 
		/// regular strings as keys. Returns map of string HashCodes and their offsets, 
		/// as well as total length of the dictionary.
		/// </summary>
		/// <param name="db">Database with list of <see cref="DBModelPart"/>.</param>
		/// <param name="length">Total length of the string block.</param>
		/// <returns>Dictionary of string HashCodes and their offsets.</returns>

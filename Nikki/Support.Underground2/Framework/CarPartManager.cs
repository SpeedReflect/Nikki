	...
	public static class CarPartManager
	{
			...
			foreach (var model in db.ModelParts.Collections)
			{
				...
				foreach (Parts.CarParts.RealCarPart realpart in model.ModelCarParts)
				{
					...
					if (realpart.Struct.Exists == eBoolean.True && realpart.Struct.Templated == eBoolean.True)
					{
						...
						length = Inject(cpstr.Geometry0LodA, length);
						length = Inject(cpstr.Geometry0LodB, length);
						length = Inject(cpstr.Geometry0LodC, length);
						length = Inject(cpstr.Geometry0LodD, length);
						length = Inject(cpstr.Geometry1LodA, length);
						length = Inject(cpstr.Geometry1LodB, length);
						length = Inject(cpstr.Geometry1LodC, length);
						length = Inject(cpstr.Geometry1LodD, length);
					}
		...
		private static string[] ReadModels(BinaryReader br, int maxlen)
		{
			...
			for (int a1 = 0; a1 < count; ++a1)
			{
				...
				foreach (var part in Parts.CarParts.UsageType.TrafficPartName)
					(result[a1] + part).BinHash();
				foreach (var part in Parts.CarParts.UsageType.SpecPartNames)
					(result[a1] + part).BinHash();
				foreach (var part in Parts.CarParts.UsageType.MiscPartsRacer)
					(result[a1] + part).BinHash();
				foreach (var part in Parts.CarParts.UsageType.MiscPartsTraffic)
					(result[a1] + part).BinHash();
			}
			return result;
		}
